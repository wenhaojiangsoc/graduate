...
kim_decomposition_longitudinal <-
  function(
    ...
    coef_normalize <- normalize_coefficient(data_reg = data %>% filter(year==year_end,get(decomp_var)==2), 
                                            data_weight = data %>% filter(year==year_end|year==year_start),
                                            dv = dv,
                                            columns = iv,
                                            normal_weight = normal_weight)
    coef_name <- coef_normalize$variable
    
    change_slope <-
      ((normalize_coefficient(data_reg = data %>% filter(year==year_end,get(decomp_var)==2), 
                              data_weight = data %>% filter(year==year_end|year==year_start),
                              dv = dv,
                              columns = iv,
                              normal_weight = normal_weight) %>% 
          filter(variable %in% coef_name) %>% dplyr::select(coef_normalize) - 
          normalize_coefficient(data_reg = data %>% filter(year==year_start,get(decomp_var)==2), 
                                data_weight = data %>% filter(year==year_end|year==year_start),
                                dv = dv,
                                columns = iv,
                                normal_weight = normal_weight) %>% 
    ...
    endowment <-
      dummy_endowment(data %>% filter(year==year_end&get(decomp_var)==2), 
                      columns=iv,
                      weight)
    ...
    change_slope <-
      ((normalize_coefficient(data_reg = data %>% filter(year==year_end,get(decomp_var)==2), 
          ...
          normalize_coefficient(data_reg = data %>% filter(year==year_start,get(decomp_var)==2), 
                                ...
                                dv = dv,
                                columns = iv,
                                normal_weight = normal_weight) %>% dplyr::select(coef_normalize))/2 - 
         (normalize_coefficient(data_reg = data %>% filter(year==year_end,get(decomp_var)==1), 
                                data_weight = data %>% filter(year==year_end|year==year_start),
                                dv = dv,
                                columns = iv,
    ...
    ## get corresponding endowment
    mean_endowment <- 
      1/2*((dummy_endowment(data %>% filter(year==year_end&get(decomp_var)==2), columns=iv,
                           weight) %>% pivot_longer(cols = colnames(endowment), 
                                                    names_to = "variable", 
                                                    values_to = "value") %>%
             arrange(variable) %>% dplyr::select(value) - 
    ...
    ## absolute change and percent being explained by each component
    D3_result <- data.frame(change = change_slope[,1] * mean_endowment[,1],
                            percent = change_slope[,1] * mean_endowment[,1] / change * 100,
                            variable = coef_normalize$variable)
    D3_result$component <- "D3"
    ...
    ## get mean of slope
    mean_slope <-
      1/4*((normalize_coefficient(data_reg = data %>% filter(year==year_end,get(decomp_var)==2), 
                                  data_weight = data %>% filter(year==year_end|year==year_start),
                                  dv = dv,
                                  columns = iv,
                                  normal_weight = normal_weight) %>% dplyr::select(coef_normalize) + 
    ...
    ## absolute change and percent being explained by each component
    D5_result <- data.frame(change = change_slope[,1] * mean_endowment[,1],
                            percent = change_slope[,1] * mean_endowment[,1] / change * 100,
                            variable = coef_normalize$variable)
    D5_result$component <- "D5"
    
    ## combine results
    result <- rbind(D2_result,D3_result,D4_result,D5_result)
    return(result)
  }
